...
class TestHotel < MiniTest::Test
  ...
  def test_checkout_guest5_from_room6
    @sheldor.check_in(@guest2)
    @sheldor.check_in(@guest3)
    @sheldor.check_in(@guest4)
    @sheldor.check_in(@guest5)
    assert_equal( [@guest2, @guest3, @guest5], @sheldor.check_out(@guest4))
